...
namespace FMCPWorldContext
{
	 ...
	 */
	UNREALMCPBRIDGECORE_API UWorld* GetEditorWorld();
	 ...
	 * the yes/no gate.
	 */
	UNREALMCPBRIDGECORE_API bool IsPIEActive();
	 ...
	 */
	UNREALMCPBRIDGECORE_API ULevel* ResolveLevelByMapPath(
		UWorld* World,
	...
	UNREALMCPBRIDGECORE_API ULevel* ResolveLevelOrNull(UWorld* World, const FString& MapPath);
	 ...
	 */
	UNREALMCPBRIDGECORE_API FString NormaliseMapPath(const FString& Raw);
	 ...
	 */
	UNREALMCPBRIDGECORE_API FString GetWorldPackagePath(UWorld* World);
}
