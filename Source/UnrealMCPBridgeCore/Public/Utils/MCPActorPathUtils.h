...
namespace FMCPActorPathUtils
{
	 ...
	 */
	UNREALMCPBRIDGECORE_API bool ParseActorPath(const FString& Raw, FActorPathParts& OutParts, FString& OutError);
	 ...
	 */
	UNREALMCPBRIDGECORE_API FString BuildActorPath(const AActor* Actor);
	 ...
	 */
	UNREALMCPBRIDGECORE_API AActor* ResolveActor(
		const FString& Raw,
		bool bRejectPIE,
		bool& OutAmbiguous,
	 ...
	 * read-only callers).
	 */
	UNREALMCPBRIDGECORE_API AActor* ResolveActorOrNull(const FString& Raw, bool bRejectPIE);
}
