...
namespace FMCPPinTypeUtils
{
	 ...
	 */
	UNREALMCPBRIDGECORE_API TSharedPtr<FJsonObject> ToJson(
	 ...
	 * subcategory paths.
	 */
	UNREALMCPBRIDGECORE_API bool FromJson(
	 ...
	 */
	UNREALMCPBRIDGECORE_API FString CategoryToWire(const FName& PinCategory);

	/**
	 * Inverse of ``CategoryToWire``. Returns ``NAME_None`` on unknown wire string.
	 */
	UNREALMCPBRIDGECORE_API FName WireToCategory(const FString& Wire);
}
