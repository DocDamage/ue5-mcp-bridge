...
namespace FMCPPageCursorUtils
{
	 ...
	 * first page).
	 */
	UNREALMCPBRIDGECORE_API FString Encode(const FMCPPageCursor& Cursor);
	 ...
	 */
	UNREALMCPBRIDGECORE_API bool Decode(const FString& TokenWire, FMCPPageCursor& OutCursor, FString& OutError);
	 ...
	 */
	UNREALMCPBRIDGECORE_API bool ValidateAgainstFilter(const FMCPPageCursor& Cursor, uint64 ExpectedFilterHash);
}
