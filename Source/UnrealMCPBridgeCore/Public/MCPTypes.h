...
/**
 * Shared log category for the entire UnrealMCPBridge plugin (both UnrealMCPBridgeCore + Tools
 * modules use ``LogMCP`` everywhere). Defined in `UnrealMCPBridgeCore`'s module .cpp.
 *
 * Phase 5 module split (2026-05-22): moved from UnrealMCPBridge.h to MCPTypes.h to break the
 * Core → Tools include cycle. Core files that previously included UnrealMCPBridge.h ONLY for
 * `LogMCP` now include MCPTypes.h instead.
 */
UNREALMCPBRIDGECORE_API DECLARE_LOG_CATEGORY_EXTERN(LogMCP, Log, All);
