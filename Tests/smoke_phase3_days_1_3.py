#!/usr/bin/env python3
"""Phase 3 Day 1 smoke — Lane B sanity probe + 2 read-only Level tools.

Scope (Day 1 only — Days 2-3 will append additional sub-tests as the surface grows):

  1. ``_phase3_lane_b_sanity`` — Lane B router works (spike 100x). Asserts no crashes; collects
     the thread_id reported by the listener thread; tracks unique thread ids observed.
...
Usage:
  python smoke_phase3_days_1_3.py [--host HOST] [--port PORT] [--lane-b-iters N] [--lane-b-only]
"""
...
def main() -> int:
    ...
    # Spike: 100 calls back-to-back. If the router demoted us to Lane A, response would arrive
    # after a tick delay (succeed but slower). If router were broken we'd see crashes / -32601.
